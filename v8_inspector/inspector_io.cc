...
InspectorIo::InspectorIo(Isolate *isolate, Platform *platform,
    ...
    : thread_(), delegate_(nullptr), state_(State::kNew), isolate_(isolate),
      thread_req_(), platform_(platform), dispatching_messages_(false),
      session_id_(0), script_name_(path), wait_for_connect_(wait_for_connect),
      host_name_(host_name), port_(0), file_path_(file_path) {
  main_thread_req_ = new AsyncAndAgent(
      {uv_async_t(), static_cast<Agent *>(isolate->GetData(4))});
