...
enum class InspectorAction { kStartSession, kEndSession, kSendMessage };
...
public:
  ...
  void ResumeStartup() { uv_sem_post(&thread_start_sem_); }
  void ServerDone() {
...
private:
  ...
  using MessageQueue = std::deque<
      std::tuple<Action, int, std::unique_ptr<v8_inspector::StringBuffer>>>;
  enum class State { kNew, kAccepting, kConnected, kDone, kError, kShutDown };
...
Utf8ToStringView(const std::string &message);
